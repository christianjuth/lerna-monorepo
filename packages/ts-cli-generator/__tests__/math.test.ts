...
const CWD = path.join(__dirname, "..", "examples", "math");
...
describe("math cli", () => {
  test("add", async () => {
    ...
    expect(result).toEqual(String(x + y));
  });
  ...
  test("square", async () => {
    ...
    expect(result).toEqual(String(x ** 2));
  });

  test("cube", async () => {
    const [x] = randomInts(1);
    const result = await promiseExec(`node . cube ${x}`, CWD);
    expect(result).toEqual(String(x ** 3));
  });

  test("sqrt", async () => {
    const [x] = randomInts(1);
    const result = await promiseExec(`node . sqrt ${x ** 2}`, CWD);
    expect(result).toEqual(String(x));
  });
