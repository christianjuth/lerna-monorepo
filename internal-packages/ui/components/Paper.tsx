...
export const elevationStyle = css<{
  ...
  ${({ theme }) => theme.darkMode("box-shadow: none;")}
`
...
const Div = styled.div<{
  ...
  ${elevationStyle}
  border-radius: ${({ $roundness }) => roundness($roundness)};
  display: flex;
...
const shadowAmbientShadowOpacity = 0.035

function createShadow(...px: number[]) {
  return [
    `${px[0]}px ${px[1]}px ${px[2]}px ${px[3]}px rgba(0,0,0,${shadowKeyUmbraOpacity})`,
    `${px[4]}px ${px[4]}px ${px[6]}px ${px[7]}px rgba(0,0,0,${shadowKeyPenumbraOpacity})`,
